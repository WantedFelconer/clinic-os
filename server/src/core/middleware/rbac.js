...
const clinicAccess = async (req, res, next) => {
  try {
    const rawClinicId = req.params.clinicId || req.params.id || req.body.clinic_id || req.query.clinic_id;
    const clinicId = typeof rawClinicId === 'string' ? rawClinicId.trim() : rawClinicId;
    const userId = req.user.id;
    const userRole = req.user.role;

    if (!clinicId || clinicId === '0' || clinicId === 'undefined' || clinicId === 'null') {
