...
const { sendOTP, sendPasswordReset } = require('../../../core/utils/email');
const { getJwtConfig } = require('../../../core/config/security');
...
setInterval(() => {
  const now = Date.now();
  for (const [email, tracker] of otpTrackerMap.entries()) {
    if (now - tracker.lastSentAt > 2 * 60 * 60 * 1000 && (!tracker.lockedUntil || now > tracker.lockedUntil)) {
