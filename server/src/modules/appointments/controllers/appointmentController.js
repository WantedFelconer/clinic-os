...
const { Service, Clinic } = require('../../clinics');
const { Patient } = require('../../patients');
const { AuditLog } = require('../../admin');
const db = require('../../../core/config/database');
const { calculateEndTime } = require('../../../core/utils/helpers');
const { validateDoctorClinicMembership } = require('../../../core/middleware/rbac');
const { ensureAppointmentModificationAllowed, normalizeDateString } = require('../../../core/utils/appointments');
const { isValidDateOnly, validateAppointmentClock } = require('../../../core/utils/dateTime');
const {
  sendAppointmentBookingNotification,
  sendAppointmentStatusNotification,
  sendAppointmentRescheduleNotification,
} = require('../../../core/utils/email');
  ...
  async reschedule(req, res, next) {
    try {
      ...
      const validation = await validateScheduleAndConflict({
        ...
        appointment_date: normalizeDateString(appointment_date || existing.appointment_date),
